...
<details>
  ...
  <p>Change to that directory where tutorial.sh is in using <code>cd</code>. Now run that script with the command <code>bash tutorial.sh</code> The tutorial should now start in your terminal! Make sure you run commands within the subfolder so use <code>cd</code> to change into subfolders. If you ever want to revert to the previous folder use <code>cd ..</code></p>
</details>
